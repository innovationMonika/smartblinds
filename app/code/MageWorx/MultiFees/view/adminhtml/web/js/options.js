    ...
    return function (config) {
        var attributeOption = {
            ...
            add: function (data, render) {
                ...
                (function($){
                    var html = $(element);
                    var priceTypeId = 'option_' + data.id + '_price_type';
                    var priceTypeSelector = '#' + priceTypeId + ' '  + 'option[value=' + data.price_type + ']';
                    html.find(priceTypeSelector).attr('selected', 'selected');
                    element = html.prop('outerHTML');
                })(jQuery);
            ...
            getOptionInputType: function () {
                var optionDefaultInputType = 'checkbox';
        ...
        if (config.isSortable) {
            jQuery(function ($) {
                $('[data-role=options-container]').sortable({
                    ...
                    axis: 'y',
                    update: function () {
                        $('[data-role=options-container] [data-role=position]').each(function (index, element) {
                            $(element).val(index + 1);
                        });
                    }
                });
