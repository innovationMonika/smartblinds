define([
  ...
  $.widget("mage.priceOptionWidthHeight", {
    ...
    _initEvents: function () {
      ...
      $window.on(
        "swatches-click",
        this._handleSystemSizeAvailability.bind(this)
      );
    ...
    _handleSystemSizeAvailability: function () {
      ...
      const smallSystemProductOptionParams = OptionParams.get(
        smallSystemProductId,
        smallSystem
      );
      ...
      const width = parseFloat(this.$valueField.data("width")),
      ...
      if (
        width > maxWidth ||
        height > maxHeight ||
        (parseFloat(smallSystemProductOptionParams.product.thickness) >= 0.5 &&
          width >= 2000)
      ) {
    ...
    _updatePlaceholders: function (value) {
      ...
      const systemPlaceholderID =
        window.jsonConfig.systemsPlaceholder[system.id];
      /* this.$widthField.attr('placeholder', this.$widthField.data('placeholder'));
            ...
            const toString = $t(' to '); */
      if (systemPlaceholderID.widthPlaceHolder !== "") {
        this.$widthField.attr(
          "placeholder",
          systemPlaceholderID.widthPlaceHolder
        );
      }
      if (systemPlaceholderID.heightPlaceHolder !== "") {
        this.$heightField.attr(
          "placeholder",
          systemPlaceholderID.heightPlaceHolder
        );
      }
    ...
    _addValidatorWidthHeight: function () {
      ...
      if (width && height) {
        ...
        if (
          ...
          systemMotor.isChainCustomerGroup == 1
        ) {
          $(
            "#option-label-control_type-" +
              systemMotor.controlTypeData["attributeId"] +
              "-item-" +
              systemMotor.controlTypeData["options"]["chain"]
          ).removeClass("disabled");
          $(
            "#option-label-control_type-" +
              systemMotor.controlTypeData["attributeId"] +
              "-item-" +
              systemMotor.controlTypeData["options"]["motor"]
          ).addClass("disabled");
    ...
    _addValidatorMethod: function () {
      ...
      $.validator.addMethod(
        ...
        function (value, element) {
          ...
          if (
            !self.$valueField.data("width") ||
            !self.$valueField.data("height")
          ) {
          ...
          if (!height || !width) {
            ...
            $(
              "#option-label-control_type-" +
                system.controlTypeData["attributeId"] +
                "-item-" +
                system.controlTypeData["options"]["chain"]
            ).removeClass("disabled");
            $(
              "#option-label-control_type-" +
                system.controlTypeData["attributeId"] +
            ...
            $(
              "#option-label-control_type-" +
                system.controlTypeData["attributeId"] +
                "-item-" +
                system.controlTypeData["options"]["motor"]
            ).addClass("disabled");
            console.log(
