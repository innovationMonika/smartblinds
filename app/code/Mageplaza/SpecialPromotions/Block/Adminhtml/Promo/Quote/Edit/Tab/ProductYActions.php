...
use Mageplaza\SpecialPromotions\Block\Adminhtml\ProductYActions as BlockYActions;
 ...
 */
class ProductYActions extends Generic implements
    TabInterface
{
    /**
     * Core registry
     ...
     * @var string
     */
    protected $_nameInLayout = 'product_y_actions_apply_to';
    ...
    /**
     * ProductYActions constructor.
     *
     * @param Context $context
     * @param Registry $registry
     * @param FormFactory $formFactory
     * @param Yesno $sourceYesno
     * @param BlockYActions $ruleActions
    ...
    protected function addTabToForm(
        $model,
        $fieldsetId = 'mp_product_y_actions_fieldset',
        $formName = 'sales_rule_form'
    ) {
        if ($id = $this->getRequest()->getParam('id')) {
            $model = $this->ruleFactory->create();
            $model->load($id);
            $model->getProductYActions()->setJsFormObject('sales_rule_formproduct_y_rule_actions_fieldset_' . $id);
        }
        $actionsFieldSetId = $model->getActionsFieldSetId($formName . 'product_y_');

        $newChildUrl = $this->getUrl(
            'sales_rule/promo_quote/newProductYActionHtml/form/' . $actionsFieldSetId,
            ['form_namespace' => $formName]
        );

        /** @var Form $form */
        $form = $this->_formFactory->create();
        $form->setHtmlIdPrefix('mp_product_y_rule_');
        ...
        $fieldset->addField(
            'mp_product_y_actions',
            'text',
            [
                'name' => 'mp_product_y_apply_to',
                'label' => __('Apply To'),
                'title' => __('Apply To'),
                'required' => true,
        ...
        $this->setActionFormName($model->getProductYActions(), $formName);
    ...
    private function setActionFormName(AbstractCondition $actions, $formName)
    {
        ...
        if ($actions->getProductYActions() && is_array($actions->getProductYActions())) {
            foreach ($actions->getProductYActions() as $condition) {
